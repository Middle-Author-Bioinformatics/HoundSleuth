...
FOLDER_PREFIX = 'midis-'
LOG_FILE = '/home/ark/MAB/midis/processed_folders.log'
...
def process_folder(folder_name):
    ...
    local_dir = f"/home/ark/MAB/midis/{folder_name}"
    print(f"Processing folder: {folder_name}")
    download_folder(BUCKET_NAME, folder_name, local_dir)

    # Run midis.sh script with folder_name as argument
    midis_script_path = '/home/ark/MAB/bin/midis-local/midis.sh'
    try:
        subprocess.run([midis_script_path, folder_name], check=True)
        print(f"Successfully processed folder {folder_name} using midis.sh.")
    except subprocess.CalledProcessError as e:
        print(f"Error running midis.sh on folder {folder_name}: {e}")
