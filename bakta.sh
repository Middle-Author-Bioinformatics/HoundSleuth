...
# **************************************************************************************************
# Run Bakta
bakta --db /home/ark/databases/bakta-1.10.3/db --keep-contig-headers --threads 16 --compliant --output $OUT --prefix ${OUT}/${input%.*} ${DIR}/${input}
...
python3 /home/ark/MAB/bin/HoundSleuth/send_email.py \
    --sender binfo@midauthorbio.com \
    --recipient ${email} \
    --subject "Your Bakta Results!" \
    --body "Hi ${name},

    Your Bakta results are available for download using the link below. The link will expire in 24 hours.

    ${url}

    Please visit https://github.com/voutcn/bakta for documentation.
